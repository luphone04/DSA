...
for i in range(E): #CREATING EDGE edge(u,v) in the graph
    u,v = map(int, input().split()) #u represent the source vertex and v represent the destination vertex 
...
# %d: Prints the vertex number (v+1 for 1-based indexing).
# %5s: Prints the color of the vertex.
# dv: Prints the distance of the vertex from the source (or "Inf" if not reachable).
# pv: Prints the parent vertex (or "None" if there is no parent).

# WHITE: A vertex is initially marked as "WHITE." This color typically signifies that the vertex has 
# not been visited or explored yet. In the context of BFS, "WHITE" vertices are unvisited and are 
# candidates to be added to the queue for exploration.

# GREY: When a "WHITE" vertex is being considered for exploration, it is marked as "GREY." In BFS,
#     this means that the vertex is currently in the queue and is awaiting processing. It is being
# visited, but its neighbors have not been explored yet.

# BLACK: Once a "GREY" vertex has been fully explored and all its neighbors have been visited, 
# it is marked as "BLACK." In BFS, "BLACK" vertices are those that have been completely processed and 
# their neighbors have been visited. They are essentially considered as fully explored and have no 
# further unvisited neighbors.